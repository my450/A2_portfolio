document.addEventListener('DOMContentLoaded', function () {
    ...
    updateSVGAttributes();
    const SVGAttributes = ["(max-width: 576px)", "(max-width: 768px)", "(max-width: 992px)"];
    SVGAttributes.forEach(breakpoint => {
    window.matchMedia(breakpoint).addEventListener("change", updateSVGAttributes);
    }); // Flag to control the radius animation
    

    circle.addEventListener('mousemove', (e) => {
        hoverText.style.display = 'block';
        hoverText.style.left = `${e.pageX + 10}px`; // Position slightly offset
    ...
    function updateFinalXPercent() {
        if (window.matchMedia("(max-width: 576px)").matches) {
            finalXPercent = 40; // Smaller screen (mobile)
        } else if (window.matchMedia("(max-width: 768px)").matches) {
    ...
    function animateArrowToPosition(startX, endX, duration = 0.8) {
        ...
        function updateArrowPosition() {
            ...
            if (elapsedTime < duration * 1000) {
                ...
                line.setAttribute('x2', `${endX}%`); // Ensure final position is set
            }
        }
    ...
    window.addEventListener('wheel', (event) => {
        ...
        if (scrollUp && !isAnimating) {
            if (currentScroll === 0 && container.style.transform === 'translateX(-100vw)') {
                ...
                slideTo('0px', 2.5); // Slide to landing-page and move arrow
    ...
    if (circle) {
        circle.addEventListener('click', () => {
            ...
            animateArrowToPosition(25, 80, 2); // Move arrow to final position
            document.body.style.overflowY = 'scroll'; // Enable vertical scrolling
        });
    }
...
document.addEventListener("DOMContentLoaded", () => {
    // Ensure the dragzone exists
    const dragzone = document.querySelector(".dragzone");
    if (!dragzone) {
        console.error("Dragzone not found in the DOM!");
        return;
    }

    // Select all pills within the dragzone
    const pills = dragzone.querySelectorAll(".pill");
    if (pills.length === 0) {
        console.error("No pills found in the dragzone!");
        return;
    }

    // Get dragzone dimensions
    const dragzoneWidth = dragzone.offsetWidth;
    const dragzoneHeight = dragzone.offsetHeight;

    // Array to keep track of positions to avoid overlap
    const occupiedPositions = [];

    pills.forEach((pill, index) => {
        // Default fallback dimensions
        const pillWidth = pill.offsetWidth || 100; // Fallback width
        const pillHeight = pill.offsetHeight || 30; // Fallback height

        // Function to generate a random position without overlap
        const getRandomPosition = () => {
            let randomLeft, randomTop;
            let overlap = true;
            while (overlap) {
                randomLeft = Math.random() * (dragzoneWidth - pillWidth);
                randomTop = Math.random() * (dragzoneHeight - pillHeight);

                // Check if the new position overlaps with any already occupied positions
                overlap = false;
                for (const position of occupiedPositions) {
                    const distanceX = Math.abs(position.left - randomLeft);
                    const distanceY = Math.abs(position.top - randomTop);
                    if (distanceX < pillWidth && distanceY < pillHeight) {
                        overlap = true;
                        break;
                    }
                }
            ...
            // Once a valid position is found, save it in occupiedPositions
        ...
        // Generate a random base rotation angle for the pill and add slight variation
        const randomRotationBase = Math.random() * 360 - 180; // Between -180 and 180 degrees
        const randomRotationVariation = (Math.random() - 0.5) * 30; // A small variation, between -15 and 15 degrees
        const randomRotation = randomRotationBase + randomRotationVariation; // Combine the base and variation

        // Apply the initial styles directly
        pill.style.position = "absolute"; // Ensure positioning works
        pill.style.left = `${left}px`; // Final horizontal position
        pill.style.top = `${top}px`; // Final vertical position
        pill.style.transform = `rotate(${randomRotation}deg)`; // Apply rotation directly

        // Apply opacity transition for the falling animation
        pill.style.opacity = "0";  // Start invisible

        // Delay each pill's animation to make the falling effect unique
        pill.style.animationDelay = `${index * 0.2}s`; // Delay each pill's animation to make it unique

        // Trigger the animation to make the pill fall
        setTimeout(() => {
            pill.style.opacity = "1"; // Make pill visible
            pill.style.transition = "transform 1s cubic-bezier(0.075, 0.82, 0.165, 1), opacity 1s cubic-bezier(0.075, 0.82, 0.165, 1)"; // Transition properties
            pill.style.transform = `translateY(0) rotate(${randomRotation}deg)`; // Final position with variation in rotation
        }, index * 200); // Delay slightly for each pill
    });
});
